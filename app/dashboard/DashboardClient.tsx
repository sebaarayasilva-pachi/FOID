  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-950">
      ...
      <main className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 min-h-0 p-4 flex flex-col gap-3 overflow-hidden border-2 border-cyan-500/60">
          <section className="grid grid-cols-2 sm:grid-cols-4 gap-3 shrink-0 items-stretch border-2 border-amber-500/60">
            <KpiCard
              title="Saldo Banco"
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col border-2 border-emerald-500/60">
            <section className="grid grid-cols-2 gap-3 flex-1 min-h-0 border-2 border-rose-500/60" style={{ gridTemplateRows: 'minmax(0, 1fr) minmax(0, 1fr)' }}>
...
function ChartCard({ title, children, compact, href, style, className }: { title: string; children: React.ReactNode; compact?: boolean; href?: string; style?: React.CSSProperties; className?: string }) {
  const card = (
    <div className={`bg-slate-900/60 rounded-lg border-2 border-violet-500/70 shadow flex flex-col min-h-0 overflow-hidden transition-colors ${href ? 'cursor-pointer hover:border-slate-600 hover:bg-slate-900/80' : ''} ${compact ? 'p-3' : 'p-6'}`}>
